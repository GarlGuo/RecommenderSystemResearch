...
\begin{document}
	...
	\begin{equation} \label{SVD++}
		\begin{split}
			...
			& + q_i^T  (p_u + |N_u|^{-.5} \sum_{j \in N_u}y_j) \\ 
			& + |R^k(i; u)|^{-.5} \sum_{j \in R^k(i; u)} (r_{u_j} - b_{u_j}) w_{ij} \\ 
			& + |N^k(i; u)|^{-.5} \sum_{j \in N^k(i; u)} c_{ij}
		\end{split}
	\end{equation}
	...
	\begin{itemize}
		\item $R_u$ all items for which user $u$ provides a rating 
		
		\item $N_u$ all items for which user $u$ provides an implicit preference
		
		\item $S^k(i)$ $k$ items rated by $u$ that are similar to item $i$
		
		\item $N^k{(i; u)} = N_u \cap S^k (i)$
		
		\item $R^k{(i; u)} = R_u \cap S^k (i)$
		
		\item $w_{ij}$ weight from $j$ to $i$
		
		\item $c_{ij}$ significance offset (indication of predictability by $j$ to $i$)
	\end{itemize}
	
	The remaining terms form the final tier, or "neighborhood tier" called by Koren, as it contributes some adjustments to the profile.
